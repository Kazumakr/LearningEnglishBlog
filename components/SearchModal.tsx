...
const SearchModal = ({ setShow, posts }: Props) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const result = posts.filter((post) => {
      return post.title.toLowerCase().match(e.target.value.toLowerCase())
    })
  ...
  return (
    <div
      ...
      tabIndex={-1}
      aria-hidden="true"
      className="h-modal fixed top-0 left-0 right-0 z-50  w-full overflow-y-auto overflow-x-hidden bg-black  bg-opacity-90 md:inset-0 md:h-full"
    >
